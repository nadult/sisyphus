...
#undef CASE
	...
	void initSoundMap() {
		...
		file_entries = findFiles(s_prefix);
...
//		printf("active: %d\n", max_sources - s_num_free_sources);
	...
	int playSound(int sound_id, game::SoundType::Type sound_type, const float3 &pos, const float3 &vel) {
		...
		alSourcei (source_id, AL_SOURCE_RELATIVE, sound_type == game::SoundType::background? AL_TRUE : AL_FALSE);	
		alSourcef(source_id, AL_ROLLOFF_FACTOR, s_sound_rolloffs[sound_type]);
		alSourcei(source_id, AL_LOOPING, sound_type == game::SoundType::looping || sound_type == game::SoundType::background? AL_TRUE : AL_FALSE);
	...
	void updateSource(int id, float loudness) {
		loudness = pow(loudness + 0.5f, 3.0);
